...
describe('Recommendation', () => {
  describe('allPossibleAssignments', () => {
    it('returns possible moves when nobody is assigned', () => {
      const allPossibleAssignments = Array.from(
        Recommendation.allPossibleAssignments({
          current: {
            entities: [{'.key': 'p1', type: 'person', location: constants.LOCATION.UNASSIGNED}],
            lanes: [],
          },
        })
      );

      expect(allPossibleAssignments).toEqual([[[['p1'], 'new-lane']]]);
    });
    ...
    it('returns possible moves when nobody is assigned but there is a lane', () => {
      ...
      expect(allPossibleAssignments).toEqual([[[['p1'], 'l1']]]);
    });
      ...
      const allPossibleAssignments = Array.from(
        Recommendation.allPossibleAssignments({
          current: {
            ...
            lanes: [{'.key': 'l1', locked: true}],
          },
        })
      );

      expect(allPossibleAssignments).toEqual([[[['p2'], 'new-lane']]]);
    });
    ...
    it('generates all possible assignments of unassigned people to lanes', () => {
      ...
      const toStringArray = (assignments, lane) =>
        assignments
          .map((as) => as.find((a) => a[1] === lane))
          .map((a) => JSON.stringify(a[0].sort()));

      expect(toStringArray(allPossibleAssignments, 'l1')).toEqual(
      ...
      expect(toStringArray(allPossibleAssignments, 'l2')).toEqual(
        expect.arrayContaining([
          '["p1","p2"]',
          '["p1","p3"]',
          '["p1"]',
    ...
    it('generates all context-preserving rotations', () => {
      ...
      const entries = allPossibleAssignments.map((as) =>
        as.map((a) => JSON.stringify([a[0].sort(), a[1]])).reduce((s, acc) => s + acc, '')
      );
      expect(_.uniq(entries).sort()).toEqual(entries.sort());
      combination(['p1', 'p2', 'p3', 'p4', 'p5'], 2).forEach((pair) => {
        if (_.isEqual(pair, ['p1', 'p2'])) {
          expect(
            allPossibleAssignments.map((as) => {
  ...
  describe('calculateMovesToBestPairing', () => {
    ...
    describe('with 3 people', () => {
      ...
      it("will not move people that are locked", () => {
        const bestPairing = Recommendation.calculateMovesToBestPairing({
          current: {
            entities: [
              {'.key': 'p1', type: 'person', location: 'l1'},
      ...
      it("will not move people that are both locked", () => {
        const bestPairing = Recommendation.calculateMovesToBestPairing({
          current: {
            entities: [
      ...
      it("will still move people from out when everyone in a lane is locked", () => {
        const bestPairing = Recommendation.calculateMovesToBestPairing({
          current: {
            entities: [
              {'.key': 'p1', type: 'person', location: 'l1', locked: true},
      ...
      it('when scores are tied, it does not always pair the same people', () => {
        ...
        while (bestPairings.length < 20) {
          bestPairings.push(
            Recommendation.calculateMovesToBestPairing({
              current: {
                entities: [
                  {'.key': 'p1', type: 'person', location: constants.LOCATION.UNASSIGNED},
                  {'.key': 'p2', type: 'person', location: constants.LOCATION.UNASSIGNED},
        ...
        const uniquePairings = _.uniqWith(
          _.isEqual,
          bestPairings.map((ps) => normalizePairing(ps))
        );
        expect(uniquePairings).toEqual(
          expect.arrayContaining([
            expect.arrayContaining([
              {lane: 'new-lane', entities: ['p1']},
              {lane: 'new-lane', entities: ['p2', 'p3']},
            ]),
            expect.arrayContaining([
              {lane: 'new-lane', entities: ['p2']},
              {lane: 'new-lane', entities: ['p1', 'p3']},
            ]),
            expect.arrayContaining([
              {lane: 'new-lane', entities: ['p3']},
              {lane: 'new-lane', entities: ['p1', 'p2']},
            ]),
          ])
        );
      });
    ...
    describe('fuzz pairing static (repro from interesting failures)', () => {
      ...
      it('fuzz 5', () => {
        ...
        const emptyLanes = board.current.lanes
          .filter((l) => !l.locked && !people.some((p) => p.location === l['.key']))
          .map((l) => l['.key']);
      ...
      it('fuzz 6', () => {
        ...
        const emptyLanes = board.current.lanes
          ...
          .map((l) => l['.key']);
        expect(
          bestPairing.some((move) => emptyLanes.includes(move.lane)) || bestPairing.length === 0
        ).toBeTruthy();
      });
      ...
      it('fuzz 9', () => {
        ...
        expect(_.flatten(bestPairing.map((p) => p.entities)).length).toBeGreaterThanOrEqual(
          board.current.entities.filter(
            (e) => e.type === 'person' && e.location === constants.LOCATION.UNASSIGNED
          ).length
        );
      });
      ...
      it('fuzz 11', () => {
        const board = require('./fixtures/board-from-fuzz-11.json');
        const bestPairing = Recommendation.calculateMovesToBestPairing(board);
        expect(bestPairing).toBeTruthy();
        expect(_.flatten(bestPairing.map((p) => p.entities)).length).toBeGreaterThanOrEqual(
          board.current.entities.filter(
        ...
        const people = board.current.entities.filter((e) => e.type === 'person');
        const lockedPeople = people.filter((p) => p.locked).map((p) => p['.key']);
        expect(bestPairing.every((move) => move.entities.every((entity) => !lockedPeople.includes(entity)))).toBeTruthy();
      })

      it('fuzz 12', () => {
        const board = require('./fixtures/board-from-fuzz-12.json');
        const bestPairing = Recommendation.calculateMovesToBestPairing(board);
        expect(bestPairing).toBeTruthy();
        expect(_.flatten(bestPairing.map((p) => p.entities)).length).toBeGreaterThanOrEqual(
          board.current.entities.filter(
      ...
      it('fuzz 13', () => {
        const board = require('./fixtures/board-from-fuzz-13.json');
        const bestPairing = Recommendation.calculateMovesToBestPairing(board);
        expect(bestPairing).toBeFalsy();
      })
    });
    ...
    describe('fuzz pairing', () => {
      for (let i = 0; i < 500; i++) {
        it(`fuzz #${i}`, () => {
          ...
          const lockedPeopleCount = randomInt(3);
          const outCount = randomInt(4);
          const lockedOutCount = randomInt(3);
          ...
          fs.writeFileSync(
            `/tmp/pairist-fuzz-pairing/board-${i}.json`,
          ...
          if (lanesCount * 2 - 1 > peopleCount) {
            ...
            assert.strictEqual(
              bestPairing,
              null,
              JSON.stringify({config, current: board.current})
            );
          } else {
            if (trackCount > 0 && peopleCount > 0) {
              const results = measureAllocations({
                current: _.cloneDeep(board.current),
                history: _.cloneDeep(board.history),
              });
...
const measureAllocations = ({current, history}) => {
  const applyAssignment = (current, pairing) => {
    ...
    return pairing.reduce((current, asst) => {
      ...
      asst.entities.forEach((movingEntity) => {
        const idx = current.entities.findIndex(
          (currentEntity) => currentEntity['.key'] === movingEntity
        );
  ...
  let nextCurrent = applyAssignment(
    current,
    Recommendation.calculateMovesToBestPairing({
      current: current,
      history: history,
  ...
  measuredHistory.forEach((h) => {
    _.values(
      _.groupBy(
        'location',
        h.filter((e) => e.type === 'person' && e.location !== 'out')
      )
    ).forEach((pairing) => {
